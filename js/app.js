...
const ALL_TEAMS = ['ketchup', 'mayo', 'cheeseburger', 'frites', 'moutarde', 'creme'];
const TEAM_LABELS = { ketchup: '🍅 KETCHUP', mayo: '🥚 MAYO', cheeseburger: '🧀 CHEESEBURGER', frites: '🍟 FRITES', moutarde: '🟡 MOUTARDE', creme: '🍦 CRÈME' };
const TEAM_COLORS = { ketchup: '#e74c3c', mayo: '#f1c40f', cheeseburger: '#e67e22', frites: '#f39c12', moutarde: '#d4ac0d', creme: '#adb5bd' };
const TEAM_TEXT_DARK = new Set(['mayo', 'frites', 'moutarde', 'creme']); // teams needing dark text
// Back-compat alias – some old code still references TEAMS; keep it pointing to active teams at runtime
let TEAMS = ALL_TEAMS.slice(0, 4);
...
function getDefaultSettings() {
  return {
    ...
    addition: { points: 4 },
    death: { timer: 0 },
    general: { teamOrder: 'random', animateur1: 'Jean-Pierre', animateur2: 'Cyril', teamCount: 4, selectedThemes: [], teamMembers: {} }
  };
}
...
function loadSettings() {
  return {
    ...
    general: {
      ...
      teamCount: parseInt(document.getElementById('set-team-count').value) || 4,
      selectedThemes: Array.from(document.querySelectorAll('#set-themes-container input[type=checkbox]:checked')).map(cb => cb.value),
      teamMembers: (gs.settings && gs.settings.general && gs.settings.general.teamMembers) || {}
    }
  };
}
...
// Returns the active teams based on current settings teamCount
function getActiveTeams() {
  const count = (gs.settings && gs.settings.general && gs.settings.general.teamCount) || 4;
  return ALL_TEAMS.slice(0, count);
}

// Build teams-preview badges on welcome screen
function buildTeamsPreview() {
  const count = (gs.settings && gs.settings.general && gs.settings.general.teamCount) || 4;
  const preview = document.getElementById('teams-preview');
  if (!preview) return;
  preview.innerHTML = ALL_TEAMS.slice(0, count).map(tm =>
    `<div class="team-badge team-${tm}">${TEAM_LABELS[tm]}</div>`
  ).join('');
}

// Build team-order select options dynamically
function buildTeamOrderOptions() {
  const count = (gs.settings && gs.settings.general && gs.settings.general.teamCount) || 4;
  const sel = document.getElementById('set-team-order');
  if (!sel) return;
  const activeTeams = ALL_TEAMS.slice(0, count);
  let opts = `<option value="random">${t('settings_order_random')}</option>`;
  const orderKeys = { ketchup: 'settings_order_ketchup', mayo: 'settings_order_mayo', cheeseburger: 'settings_order_cheese', frites: 'settings_order_frites', moutarde: 'settings_order_moutarde', creme: 'settings_order_creme' };
  activeTeams.forEach(tm => {
    const key = orderKeys[tm];
    opts += `<option value="${tm}-first">${t(key)}</option>`;
  });
...
// Build active teams preview in settings
function buildSettingsTeamsPreview() {
  const sel = document.getElementById('set-team-count');
  const count = sel ? parseInt(sel.value) : 4;
  const preview = document.getElementById('teams-active-preview');
  if (!preview) return;
  preview.innerHTML = ALL_TEAMS.slice(0, count).map(tm =>
    `<span style="background:${TEAM_COLORS[tm]};color:${TEAM_TEXT_DARK.has(tm)?'#333':'#fff'};padding:5px 12px;border-radius:20px;font-family:var(--font-heading);font-size:13px">${TEAM_LABELS[tm]}</span>`
  ).join('');
  buildTeamOrderOptions();
}
...
// ======================== PLAYER DRAW ========================
function openPlayerDrawModal() {
  const container = document.getElementById('player-fields-container');
  container.innerHTML = '';
  // Pre-fill existing members if any
  const existing = (gs.settings.general && gs.settings.general.teamMembers) || {};
  const allPlayers = Object.values(existing).flat();
  const initCount = Math.max(allPlayers.length, 2);
  for (let i = 0; i < initCount; i++) addPlayerField(allPlayers[i] || '');
  document.getElementById('player-draw-preview').style.display = 'none';
  document.getElementById('player-draw-modal-btns').style.display = 'none';
  document.getElementById('modal-player-draw').style.display = 'flex';
}
...
function addPlayerField(value) {
  ...
  row.innerHTML = `<span style="min-width:22px;color:var(--color-text-dim);font-size:13px">${idx}.</span>
    <input type="text" maxlength="20" placeholder="Prénom du participant" value="${value || ''}" style="flex:1;padding:8px 12px;border-radius:8px;border:2px solid #1a3a6e;background:var(--color-bg-light);color:var(--color-text);font-size:15px">
    <button onclick="this.parentElement.remove();renumberPlayerFields()" style="background:none;border:none;color:var(--color-text-dim);cursor:pointer;font-size:18px">✕</button>`;
  row.querySelector('input').addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); addPlayerField(); setTimeout(() => container.lastElementChild && container.lastElementChild.querySelector('input').focus(), 50); } });
  container.appendChild(row);
}

function renumberPlayerFields() {
  document.querySelectorAll('#player-fields-container > div > span').forEach((span, i) => span.textContent = (i + 1) + '.');
}

function runPlayerDraw() {
  const inputs = Array.from(document.querySelectorAll('#player-fields-container input')).map(i => i.value.trim()).filter(Boolean);
  if (inputs.length < 2) { showToast('Ajoutez au moins 2 participants !'); return; }
  ...
  const result = {};
  teams.forEach(tm => result[tm] = []);
  shuffled.forEach((name, idx) => result[teams[idx % teams.length]].push(name));
  // Store temporarily for preview
  gs._pendingTeamMembers = result;
  // Render preview
  const preview = document.getElementById('player-draw-preview');
  preview.innerHTML = '<h3 style="margin-bottom:12px;font-family:var(--font-heading)">🎉 Résultat du tirage</h3>' +
    teams.map(tm => `<div style="margin-bottom:8px;padding:8px 12px;border-radius:8px;background:${TEAM_COLORS[tm]};color:${TEAM_TEXT_DARK.has(tm)?'#333':'#fff'};font-family:var(--font-heading)">
      <strong>${TEAM_LABELS[tm]}</strong> : ${result[tm].join(', ') || '<em>—</em>'}</div>`).join('');
  preview.style.display = 'block';
  document.getElementById('player-draw-modal-btns').style.display = 'block';
}

function confirmPlayerDraw() {
  if (!gs._pendingTeamMembers) return;
  if (!gs.settings.general) gs.settings.general = getDefaultSettings().general;
  gs.settings.general.teamMembers = gs._pendingTeamMembers;
  localStorage.setItem(LS_SETTINGS, JSON.stringify(gs.settings));
  gs._pendingTeamMembers = null;
  document.getElementById('modal-player-draw').style.display = 'none';
  renderPlayerDrawResult(gs.settings.general.teamMembers);
  buildScoreboard();
  showToast('Équipes enregistrées !');
}

function renderPlayerDrawResult(members) {
  const el = document.getElementById('player-draw-result');
  if (!el) return;
  const teams = getActiveTeams();
  const hasAny = teams.some(tm => members[tm] && members[tm].length);
  if (!hasAny) { el.style.display = 'none'; return; }
  el.style.display = 'block';
  el.innerHTML = teams.filter(tm => members[tm] && members[tm].length).map(tm =>
    `<div style="margin-bottom:6px;padding:6px 10px;border-radius:8px;background:${TEAM_COLORS[tm]};color:${TEAM_TEXT_DARK.has(tm)?'#333':'#fff'};font-size:13px;font-family:var(--font-heading)">
      <strong>${TEAM_LABELS[tm]}</strong> : ${members[tm].join(', ')}</div>`).join('');
}

function showThemePickModal() {
  const container = document.getElementById('modal-themes-container');
  if (!container) { confirmStartGame(); return; }
  const themes = typeof getAvailableThemes === 'function' ? getAvailableThemes() : [];
  if (!themes.length) { confirmStartGame(); return; }
  const currentThemes = (gs.settings && gs.settings.general && gs.settings.general.selectedThemes) || [];
  const useAll = !currentThemes.length;
  container.innerHTML = themes.map(theme => {
    const checked = (useAll || currentThemes.includes(theme)) ? 'checked' : '';
    return `<label style="display:flex;align-items:center;gap:10px;cursor:pointer;font-size:16px"><input type="checkbox" value="${theme}" ${checked} style="width:20px;height:20px;accent-color:var(--color-accent)"><span>${theme}</span></label>`;
  }).join('');
  document.getElementById('modal-theme-pick').style.display = 'flex';
}

function applyThemesAndStart() {
  document.getElementById('modal-theme-pick').style.display = 'none';
  const themes = Array.from(document.querySelectorAll('#modal-themes-container input[type=checkbox]:checked')).map(cb => cb.value);
  if (!gs.settings) gs.settings = getDefaultSettings();
  if (!gs.settings.general) gs.settings.general = getDefaultSettings().general;
  gs.settings.general.selectedThemes = themes;
  // Sync the settings screen checkboxes
  document.querySelectorAll('#set-themes-container input[type=checkbox]').forEach(cb => {
    cb.checked = !themes.length || themes.includes(cb.value);
  });
  confirmStartGame();
}
...
function continueGame() {
  ...
  // Sync TEAMS back-compat alias with restored teamOrder
  if (gs.teamOrder && gs.teamOrder.length) TEAMS = [...new Set([...gs.teamOrder, ...ALL_TEAMS])].filter(tm => gs.teamOrder.includes(tm));
  // Rebuild scoreboard DOM
  buildScoreboard();
  // Reload settings
...
function startGame() {
  if (typeof editorInjectCustomQuestions === 'function') editorInjectCustomQuestions();
  gs.settings = loadSettings();
  localStorage.setItem(LS_SETTINGS, JSON.stringify(gs.settings));
  const activeTeams = getActiveTeams();
  TEAMS = activeTeams; // keep back-compat alias in sync
  gs.scores = {}; activeTeams.forEach(t => gs.scores[t] = 0);
  gs.roundHistory = {};
  ROUNDS.forEach(r => { gs.roundHistory[r] = {}; activeTeams.forEach(t => gs.roundHistory[r][t] = 0); });
  gs.questionsUsed = {};
  ...
  buildScoreboard(); updateScoreboard(); showScreen('screen-game');
  persistGameState();
  ...
  if (order === 'random') {
    ...
    const idx = activeTeams.indexOf(first);
    gs.teamOrder = idx !== -1 ? [...activeTeams.slice(idx), ...activeTeams.slice(0, idx)] : [...activeTeams];
    updateScoreboard();
...
function showTeamDrawModal() {
  const modal = document.getElementById('modal-team-draw');
  const slots = document.getElementById('draw-team-slots');
  const activeTeams = getActiveTeams();
  const ordinals = [t_fn('ordinal_1'), t_fn('ordinal_2'), t_fn('ordinal_3'), t_fn('ordinal_4'), '5ème', '6ème'];
  slots.innerHTML = activeTeams.map((_, i) =>
    `<div class="draw-slot"><span class="draw-slot-label">${ordinals[i]}</span><span class="draw-slot-value" id="draw-val-${i+1}">?</span></div>`
  ).join('');
...
function runTeamDrawAnimation() {
  ...
  const allLabels = activeTeams.map(tm => TEAM_LABELS[tm]);
  let slot = 0;
  function revealNext() {
    if (slot >= activeTeams.length) { setTimeout(() => document.getElementById('btn-team-draw-done').style.display = 'inline-flex', 400); return; }
    ...
    const iv = setInterval(() => {
      ...
      if (++cycles >= max) {
        ...
        el.style.background = TEAM_COLORS[team]; el.style.color = TEAM_TEXT_DARK.has(team) ? '#333' : '#fff';
...
function showInterRoundRecap() {
  ...
  // Build ranking
  const sorted = gs.teamOrder.map(tm => ({ team: tm, score: gs.scores[tm] })).sort((a, b) => b.score - a.score);
...
function loadNuggetsQuestion() {
  ...
  showQuestionDrawAnimation(questions, level, team, (question) => {
    ...
    // Sync toggle button to global app language
    const curLang = getLang();
    const toggleEl = document.getElementById('lang-toggle-nuggets');
    if (toggleEl) toggleEl.querySelectorAll('.lang-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.lang === curLang));
    ...
    document.getElementById('nuggets-q-text').textContent = qText(question);

    // Theme badge
    const themeBadgeN = document.getElementById('nuggets-theme-badge');
    if (themeBadgeN) {
      if (question.theme) {
        themeBadgeN.textContent = typeof themeText === 'function' ? themeText(question.theme) : question.theme;
        themeBadgeN.style.display = 'inline-block';
      } else {
        themeBadgeN.style.display = 'none';
      }
    }
...
function showQuestionDrawAnimation(questions, level, team, callback) {
  ...
  strip.innerHTML = '';
  const allQ = questions.map(q => qText(q));
  for (let i = 0; i < 35; i++) {
  ...
  final.textContent = qText(win); strip.appendChild(final);
...
function renderNuggetsQCM(question) {
  ...
  container.innerHTML = '';
  const choices = qChoices(question);
  if (!choices) return;
  ...
  indices.forEach((origIdx, displayIdx) => {
    ...
    div.textContent = `${String.fromCharCode(65 + displayIdx)}. ${choices[origIdx]}`;
...
function nuggetsPick(el, selected, correct, container) {
  ...
  setTimeout(() => {
    ...
    if (isCorrect) {
      ...
      const correctText = qChoices(gs.currentQuestion)[gs.currentQuestion.answer];
    ...
    if (isLastInLevel && isLastLevel) {
      document.getElementById('btn-next-question').style.display = 'none';
      const btnNR = document.getElementById('btn-next-round');
      btnNR.textContent = t('btn_next_round');
      btnNR.style.display = 'inline-flex';
    } else {
      const btnNQ = document.getElementById('btn-next-question');
      btnNQ.textContent = t('btn_next_question');
      btnNQ.style.display = 'inline-flex';
      document.getElementById('btn-next-round').style.display = 'none';
    }
...
function showMenusFinalOrder(orderedTeams) {
  ...
  const labels = [t('ordinal_1'), t('ordinal_2'), t('ordinal_3'), t('ordinal_4'), '5ème', '6ème'];
...
function drawWheel(teams, angle) {
  ...
  const COLORS_MAP = { ketchup: '#e74c3c', mayo: '#f1c40f', cheeseburger: '#e67e22', frites: '#f39c12', moutarde: '#d4ac0d', creme: '#adb5bd' };
  const LABELS_MAP = { ketchup: '🍅 KETCHUP', mayo: '🥚 MAYO', cheeseburger: '🧀 CHEESE', frites: '🍟 FRITES', moutarde: '🟡 MOUTARDE', creme: '🍦 CRÈME' };
...
function setMenusPickLang(lang) {
  setLang(lang);
  // Update active state on both pick-phase and play-phase toggles
  ['lang-toggle-menus-pick', 'lang-toggle-menus'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.querySelectorAll('.lang-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.lang === lang));
  });
  renderMenuGrid();
}
...
function renderMenuGrid() {
  ...
  grid.innerHTML = '';
  // Sync toggle active state to current lang
  const curLang = getLang();
  const pickToggle = document.getElementById('lang-toggle-menus-pick');
  if (pickToggle) pickToggle.querySelectorAll('.lang-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.lang === curLang));
  gs.menusAvailable.forEach((name, idx) => {
    ...
    const label = typeof menuTitle === 'function' ? menuTitle(name) : name.replace(/^[^\s]+\s+Menu\s+/, '');
...
function loadMenuQuestion() {
  ...
  document.getElementById('menus-play-menu-name').textContent = typeof menuTitle === 'function' ? menuTitle(menuName) : menuName;
  document.getElementById('menus-q-number').textContent = t('menus_q_label', gs.menusCurrentQ + 1, questions.length);
  document.getElementById('menus-q-text').textContent = qText(q);
  highlightActiveTeam(team);

  // Theme badge
  const themeBadgeM = document.getElementById('menus-theme-badge');
  if (themeBadgeM) {
    if (q.theme) {
      themeBadgeM.textContent = typeof themeText === 'function' ? themeText(q.theme) : q.theme;
      themeBadgeM.style.display = 'inline-block';
    } else {
      themeBadgeM.style.display = 'none';
    }
  }

  // Show answer buttons, hide next buttons, feedback and answers reveal
  ...
  const answersRevealEl = document.getElementById('menus-answers-reveal');
  if (answersRevealEl) answersRevealEl.style.display = 'none';
...
function menusAnswer(isCorrect) {
  ...
  // Show possible answers (reponse_fr / reponse_en)
  const answersReveal = document.getElementById('menus-answers-reveal');
  if (answersReveal) {
    const curQ = gs.currentQuestion;
    const lang = getLang();
    const answers = curQ && (lang === 'en' ? (curQ.reponse_en || curQ.reponse_fr) : (curQ.reponse_fr || curQ.reponse_en));
    if (answers && answers.length > 0) {
      const title = lang === 'en' ? 'Possible answers' : 'Réponses possibles';
      answersReveal.innerHTML = `<div class="mar-title">${title}</div><ul class="mar-list">${answers.map(a => `<li>${a}</li>`).join('')}</ul>`;
      answersReveal.style.display = 'block';
    } else {
      answersReveal.style.display = 'none';
    }
  }
  ...
  if (isLastQ && isLastTeam) {
    document.getElementById('btn-menus-next').style.display = 'none';
    const btnMNR = document.getElementById('btn-menus-next-round');
    btnMNR.textContent = t('btn_next_round');
    btnMNR.style.display = 'inline-flex';
  } else if (isLastQ) {
    ...
    const btnMN = document.getElementById('btn-menus-next');
    btnMN.textContent = t('btn_next_question');
    btnMN.style.display = 'inline-flex';
    document.getElementById('btn-menus-next-round').style.display = 'none';
  }
}
...
function finishAddition() {
  ...
  const TEAM_ICONS = { ketchup: '🍅', mayo: '🥚', cheeseburger: '🧀', frites: '🍟', moutarde: '🟡', creme: '🍦' };
...
function startDeath() {
  ...
  banner.style.color = TEAM_TEXT_DARK.has(winner) ? '#333' : '#fff';
...
function deathPickQuestion() {
  ...
  document.getElementById('death-q-text').textContent = qText(q);
  document.getElementById('death-answer').style.display = 'none';
...
function deathShowAnswer() {
  ...
  el.textContent = t('death_answer', qAnswer(gs.currentQuestion));
  el.style.display = 'block';
}
...
function setQuestionDisplayLang(zone, lang) {
  const q = gs.currentQuestion;
  if (!q) return;
  // Update active state on the toggle buttons
  const toggleId = zone === 'nuggets' ? 'lang-toggle-nuggets' : zone === 'menus' ? 'lang-toggle-menus' : 'lang-toggle-death';
  const toggleEl = document.getElementById(toggleId);
  if (toggleEl) toggleEl.querySelectorAll('.lang-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.lang === lang));

  if (zone === 'nuggets') {
    document.getElementById('nuggets-q-text').textContent = lang === 'en' ? (q.en || q.fr) : (q.fr || q.en);
    // Update theme badge
    const nThemeBadge = document.getElementById('nuggets-theme-badge');
    if (nThemeBadge && q.theme) {
      nThemeBadge.textContent = typeof themeText === 'function' ? themeText(q.theme) : q.theme;
    }
    const choices = lang === 'en' && q.choices_en ? q.choices_en : q.choices_fr;
    if (choices) {
      document.getElementById('nuggets-qcm-choices').querySelectorAll('.qcm-choice').forEach(el => {
        const letter = el.textContent[0];
        const origIdx = parseInt(el.dataset.origidx);
        if (!isNaN(origIdx)) el.textContent = `${letter}. ${choices[origIdx]}`;
      });
    }
  } else if (zone === 'menus') {
    document.getElementById('menus-q-text').textContent = lang === 'en' ? (q.en || q.fr) : (q.fr || q.en);
    // Update theme badge
    const mThemeBadge = document.getElementById('menus-theme-badge');
    if (mThemeBadge && q.theme) {
      mThemeBadge.textContent = typeof themeText === 'function' ? themeText(q.theme) : q.theme;
    }
    // Update answers reveal if visible
    const answersRevealLang = document.getElementById('menus-answers-reveal');
    if (answersRevealLang && answersRevealLang.style.display !== 'none') {
      const answers = lang === 'en' ? (q.reponse_en || q.reponse_fr) : (q.reponse_fr || q.reponse_en);
      if (answers && answers.length > 0) {
        const title = lang === 'en' ? 'Possible answers' : 'Réponses possibles';
        answersRevealLang.innerHTML = `<div class="mar-title">${title}</div><ul class="mar-list">${answers.map(a => `<li>${a}</li>`).join('')}</ul>`;
      }
    }
    // Update menu title in banner
    const menuName = gs.menusPicked && gs.menusPlayOrder ? gs.menusPicked[gs.menusPlayOrder[gs.menusPlayIdx]] : null;
    if (menuName) document.getElementById('menus-play-menu-name').textContent = typeof menuTitle === 'function' ? menuTitle(menuName) : menuName;
    // Update CORRECT / INCORRECT buttons
    const btnC = document.getElementById('btn-menus-correct');
    const btnI = document.getElementById('btn-menus-incorrect');
    if (btnC) btnC.textContent = t('btn_correct');
    if (btnI) btnI.textContent = t('btn_incorrect');
    // Update navigation buttons if visible
    const btnNext = document.getElementById('btn-menus-next');
    if (btnNext && btnNext.style.display !== 'none') btnNext.textContent = t('btn_next_question');
    const btnNextRound = document.getElementById('btn-menus-next-round');
    if (btnNextRound && btnNextRound.style.display !== 'none') btnNextRound.textContent = t('btn_next_round');
    // Sync toggle active state
    const toggleEl = document.getElementById('lang-toggle-menus');
    if (toggleEl) toggleEl.querySelectorAll('.lang-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.lang === lang));
  } else if (zone === 'death') {
    document.getElementById('death-q-text').textContent = lang === 'en' ? (q.en || q.fr) : (q.fr || q.en);
  }
}

// keep legacy alias in case anything still calls the old name
function toggleQuestionLang(zone) { setQuestionDisplayLang(zone, getLang() === 'fr' ? 'en' : 'fr'); }
...
// Rebuild scoreboard DOM from scratch based on active teams
function buildScoreboard() {
  const bar = document.getElementById('scoreboard-bar');
  if (!bar) return;
  bar.innerHTML = '';
  const activeTeams = getActiveTeams();
  const members = (gs.settings && gs.settings.general && gs.settings.general.teamMembers) || {};
  activeTeams.forEach(tm => {
    const el = document.createElement('div');
    el.className = `team-score team-${tm}`;
    el.id = `score-${tm}`;
    const playerList = members[tm] && members[tm].length ? members[tm].join('<br>') : '';
    const tooltip = playerList ? `<div class="members-tooltip">${playerList}</div>` : '';
    el.innerHTML = `<span class="team-icon">${TEAM_LABELS[tm].split(' ')[0]}</span><span class="team-name">${TEAM_LABELS[tm].split(' ').slice(1).join(' ')}</span><span class="team-points" id="pts-${tm}">0</span>${tooltip}`;
    bar.appendChild(el);
  });
}

function updateScoreboard() {
  getActiveTeams().forEach(tm => {
    const el = document.getElementById('pts-' + tm);
    if (el) el.textContent = gs.scores[tm] || 0;
  });
  // Reorder scoreboard bar to match team draw order
  if (gs.teamOrder && gs.teamOrder.length) {
    const bar = document.getElementById('scoreboard-bar');
    if (bar) gs.teamOrder.forEach(tm => {
      const el = document.getElementById('score-' + tm);
...
function highlightActiveTeam(team) {
  ...
  if (el) el.classList.add('active-team');
}
...
function showResults() {
  ...
  persistGameState();
  const sorted = TEAMS.map(tm => ({ team: tm, score: gs.scores[tm] })).sort((a, b) => b.score - a.score);
  const heights = [250, 190, 140, 100, 80, 60]; const medals = ['🥇','🥈','🥉','4️⃣','5️⃣','6️⃣'];
...
function filterDBByThemes(db, themes) {
  if (!themes || !themes.length) return db;
  const themeSet = new Set(themes);
  const matchQ = q => !q.theme || themeSet.has(q.theme);
  const nuggets = {};
  Object.entries(db.nuggets).forEach(([k, v]) => { nuggets[k] = v.filter(matchQ); });
  const menus = {};
  Object.entries(db.menus).forEach(([k, v]) => { const f = v.filter(matchQ); if (f.length) menus[k] = f; });
  return {
    nuggets,
    sel_ou_poivre: db.sel_ou_poivre.filter(matchQ),
    menus,
    addition: db.addition.filter(matchQ),
    burger_de_la_mort: db.burger_de_la_mort.filter(matchQ)
  };
}

function populateThemePicker(selectedThemes) {
  const container = document.getElementById('set-themes-container');
  if (!container) return;
  const themes = typeof getAvailableThemes === 'function' ? getAvailableThemes() : [];
  const active = (selectedThemes && selectedThemes.length) ? new Set(selectedThemes) : null;
  container.innerHTML = themes.map(theme => {
    const checked = (!active || active.has(theme)) ? 'checked' : '';
    return `<label style="display:flex;align-items:center;gap:8px;cursor:pointer"><input type="checkbox" value="${theme}" ${checked} style="width:18px;height:18px;accent-color:var(--color-accent)"><span>${theme}</span></label>`;
  }).join('');
}
...
function getQuestionsDB() {
  ...
  if (typeof QUESTIONS_DB_I18N !== 'undefined' && QUESTIONS_DB_I18N[getLang()]) {
    db = QUESTIONS_DB_I18N[getLang()];
  } else {
    db = QUESTIONS_DB;
  }
  const themes = gs.settings && gs.settings.general && gs.settings.general.selectedThemes;
  if (themes && themes.length) return filterDBByThemes(db, themes);
  return db;
}
...
function applyStaticTranslations() {
  ...
  const cardData = [
    { title: 'settings_teams_title', desc: 'settings_teams_desc', labels: ['settings_team_count_label'] },
  ...
  // Settings team order select — rebuilt dynamically (respects teamCount + lang)
  buildTeamOrderOptions();
  ...
  // Draw slot labels — the modal is rebuilt dynamically, so just update if slots exist
  const allOrdinals = [t('ordinal_1'), t('ordinal_2'), t('ordinal_3'), t('ordinal_4'), '5ème', '6ème'];
  document.querySelectorAll('.draw-slot-label').forEach((el, i) => { if (allOrdinals[i]) el.textContent = allOrdinals[i]; });
...
function applySettingsToForm(s) {
  ...
  const teamCount = s.general.teamCount || 4;
  const tcEl = document.getElementById('set-team-count');
  if (tcEl) tcEl.value = teamCount;
  buildTeamOrderOptions();
  const toEl = document.getElementById('set-team-order');
  if (toEl) {
    const orderVal = s.general.teamOrder || 'random';
    if ([...toEl.options].some(o => o.value === orderVal)) toEl.value = orderVal;
    else toEl.value = 'random';
  }
  ...
  buildSettingsTeamsPreview();
  populateThemePicker(s.general.selectedThemes || []);
  renderPlayerDrawResult(s.general.teamMembers || {});
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  updateContinueButton();
  applyLang();
  // Hookup team count change listener
  const tcEl = document.getElementById('set-team-count');
  if (tcEl) tcEl.addEventListener('change', buildSettingsTeamsPreview);
});
